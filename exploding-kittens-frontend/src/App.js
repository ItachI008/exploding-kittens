  ...
  const handleCardFlip = (index, cardType) => {
    setTimeout(() => {
      switch (cardType) {
        case 'defuse':
          setCount(count + 1);
          setToastMessage('Meow! You can stop the BOOM now!');
          break;
        case 'cat':
        ...
        case 'shuffle':
          setCount(0);
          setToastMessage('All Cards are shuffled, start from the beginning Meow');
          generateCards();
          return;
        case 'exploding':
          if (count > 0) {
            setCount(count - 1);
            setToastMessage('Meow! You defused the bomb, but be cautious!');
          } else {
            setToastMessage('No defuse card , so Game over Meow.');
